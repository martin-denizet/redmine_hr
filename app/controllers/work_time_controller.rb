class WorkTimeController < ApplicationController
  ...
  def prepare_values
    ...
    #Edit
    holidays = WtHolidays.find(:all,
      :conditions=>["holiday>:begining and holiday<:end and deleted_on is null",{:begining=>@first_date, :end=>@last_date}])
    @month_days = Hash.new
    (@first_date..@last_date).each do |date|
      ...
      styles_array=[]
      styles_array.push("wt_calendar-saturday") if date.wday==5
      styles_array.push("wt_calendar-sunday") if date.wday==6
  ...
  def issue_pos
    ...
    # 表示チケット順序変更求処理
    if params.key?("issue_pos") && params[:issue_pos] =~ /^(.*)_(.*)$/ then
      tid = $1.to_i
      dst = $2.to_i
  ...
  def add_issue_relay
    ################################### チケット付け替え関係処理
    if params.key?("issue_relay") && params[:issue_relay]=~/^(.*)_(.*)$/ then
      if User.current.allowed_to?(:edit_work_time_total, @project) then
        ...
        while anc_id != 0 do
          ...
          relay = WtIssueRelay.find(:first, :conditions=>["issue_id=:i",{:i=>anc_id}])
          break if !relay
          anc_id = relay.parent
        end

        if anc_id != child_id then
          relay = WtIssueRelay.find(:first, :conditions=>["issue_id=:i",{:i=>child_id}])
          if relay then
  ...
  def change_issue_position
    # 重複削除と順序の正規化
    if order_normalization(WtIssueRelay, :issue_id, :order=>"position") then
      @message = '<div style="background:#faa;">Warning: normalize WtIssueRelay</div>'
      return
    end
    ...
    if params.key?("issue_pos") && params[:issue_pos]=~/^(.*)_(.*)$/ then
      if User.current.allowed_to?(:edit_work_time_total, @project) then
        issue_id = $1.to_i
        dst = $2.to_i
        relay = WtIssueRelay.find(:first, :conditions=>["issue_id=:i",{:i=>issue_id}])
        if relay then
          if relay.position > dst then # 前に持っていく場合
            tgts = WtIssueRelay.find(:all, :conditions=>
  ...
  def calc_total
    ...
    relay = Hash.new
    WtIssueRelay.find(:all).each do |i|
    ...
    TimeEntry.find(:all, :conditions =>
      ...
      while true do
        ...
        if !(relay.key?(parent_iid)) then
          ...
          WtIssueRelay.create(:issue_id=>parent_iid, :position=>relay.size, :parent=>0)
        end
